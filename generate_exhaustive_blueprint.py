...
def generate_exhaustive_blueprint():
    ...
    with open(filepath, "w", encoding="utf-8") as f:
        ...
        # LOOP THROUGH 7 PAGES AND FILL OUT SECTIONS 1-14
        pages = [
            {
                "name": "Landing Page",
                "route": "/",
        ...
        f.write("# PAGE-BY-PAGE SPECIFICATIONS\n\n")

        for i, page in enumerate(pages):
            f.write(f"## PAGE: {page['name']} (`{page['route']}`)\n\n")
            ...
            f.write("### 3. Information Architecture & Layout Structure\n")
        ...
        # CONTINUING GLOBAL SECTIONS
        f.write("## 18. Component Inventory\n")
        f.write("| Component Name | Pages Used | Variants | Reusable | Dependencies |\n")
        f.write("| --- | --- | --- | --- | --- |\n")
        f.write("| Button | All | Primary, Secondary, Ghost, Outline | Yes | Lucide Icons |\n")
        f.write("| Navbar | All | Default, Mobile Collapsed | Yes | Framer Motion |\n")
        f.write("| Premium Card | Schemes, ID, Profile | Default, Hover Glow | Yes | Tailwind Utilities |\n")
        f.write("| BackgroundSystem | All | Noise overlay, gradient orbs | Yes | CSS variables |\n\n")

        f.write("## 19. Complete User Journeys\n")
        f.write("### Journey 1: Find and Apply for a Scheme\n")
        f.write("Landing Page -> Clicks 'Explore Schemes' -> Views /schemes -> Filters by Category 'Agriculture' -> Clicks Scheme Card -> Reads /schemes/[id] requirements -> Clicks 'Apply Now' -> Shows Auth Modal (if unauthenticated) -> Complete Application -> Redirects to Profile Tracker.\n\n")

        f.write("## 20. Interaction Map\n")
        f.write("- **Component:** FeatureCard -> **Interaction:** Hover -> **Result:** Arrow right translation, inner border glow opacity 100.\n")
        f.write("- **Component:** Mobile Menu Toggle -> **Interaction:** Click -> **Result:** Dropdown menu translates Y from top, background blur applied.\n\n")

        f.write("## 21. Data Mapping\n")
        f.write("- **Mock Data Strategy:** All data currently resides in `@/lib/data.ts`. Real implementation requires swapping arrays for `fetch` calls in React Server Components or `SWR`/`React Query` for client components.\n\n")

        f.write("## 22. States Documentation\n")
        f.write("- **Loading:** Skeleton loaders used for data fetching on dynamic routes.\n")
        f.write("- **Empty:** Glass pane with 'No results found' icon and reset filters button.\n")
        f.write("- **Error:** 404 Page Not Found with 'Return Home' button.\n\n")

        f.write("## 23. Responsive Behaviour\n")
        f.write("- **Desktop:** Multi-column grids (Bento grid 3-col, Scheme grid 3-col).\n")
        f.write("- **Tablet:** 2-column grids.\n")
        f.write("- **Mobile:** Single column stacked. Global navigation collapses into a hamburger menu. Hover states disabled on mobile (requires tap).\n\n")
        ...
        f.write("## 26. Edge Cases\n")
        f.write("- **Offline:** Next.js offline fallback or browser default.\n")
        f.write("- **API Failure:** Displays 'Failed to load schemes. Please try again later.'\n")
        f.write("- **Session Expired:** Automatically logs out and redirects to `/` with AuthModal opened.\n\n")

        f.write("## 27. Screenshot References\n")
        f.write("- (Simulated) Check individual route definitions for expected structural boundaries.\n\n")

        f.write("## 29. Screen Hierarchy\n")
        f.write("All screens follow a root `layout.tsx` injection which guarantees the presence of `<BackgroundSystem>` (z:-10) and `<Navbar>` (z:50), pushing page-specific `<main>` content below the fixed header.\n\n")
